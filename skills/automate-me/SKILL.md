...
Look recursively for `*-mode/SKILL.md` matching the user's handle, in the repo's skills directory and in the user-level store at `~/.agents/skills/`. Mode skills can live in a personal category directory, not only at the top level. If one exists, confirm intent with a structured question (unless they already said "update my skill" or similar):
...
Locate transcripts before fanning out, per the transcript reference (the `poteto-mode` skill, `references/transcripts.md`).
...
Mining misses intent that hasn't come up yet. Ask with a structured multiple-choice question rather than making the user type from scratch. Lower cognitive load, higher hit rate.
...
Use the **writing-for-agents** skill to author it. Placement:

- Path: preserve an existing mode skill's category. For a new mode, place it under the repo's skills directory, or in the user-level store at `~/.agents/skills/<handle>-mode/` when the user wants it available everywhere.
...
Apply the **unslop** skill and **writing-for-agents**'s guidelines to every line. Both apply to any agent-read prose, not just skills.
...
- The **writing-for-agents** skill, for authoring process and writing guidelines.
