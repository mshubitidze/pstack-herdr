...
Use the `architect runners` roster, one runner per entry. The panel's value is family diversity, so name it in the reply when the roster collapses to one family.
