...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles comments from an automated PR reviewer. Pullfrog is the bot in use here; the rubric holds for any of them. The goal is not to ignore the bot by default. The goal is to stop treating every comment as a required code change.
