---
name: control-ui
description: Drive and inspect a web, IDE, or Electron UI with agent-browser, falling back to CDP for deep instrumentation. Use for local UI verification, screenshots, accessibility snapshots, perf profiles, visual diffs, or reproducing UI bugs.
---
...
3. For a web app, open the local URL with `agent-browser`, or the repo's existing browser tooling when it has one.
...
`agent-browser` drives web and Electron surfaces. It needs no project dependency and works from any agent kind, so prefer it unless the repo already has a harness that knows the app's startup and auth.
...
`snapshot -i` returns the interactive accessibility tree with a ref per element, which is a few lines where a screenshot is tens of kilobytes. Use it to decide what to do next and save screenshots for evidence a human reads.

Refs belong to one session's most recent snapshot, so re-snapshot after anything that changes the page. A stale ref fails with `Unknown ref`, which is the harness telling you the page moved.

Always pass `--session`. Each one is an isolated browser with its own cookies, storage and auth, which is what lets parallel verifiers run without fighting over one profile. Close every session you open.
...
CDP covers what `agent-browser` does not: CPU profiles, heap snapshots, network throttling, and rendering emulation. Reach for it when you need instrumentation rather than interaction.
