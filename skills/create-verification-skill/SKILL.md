...
## 0. Resolve where the skill goes

Look at what the repo already uses for project-local skills and follow it. If the repo has none, default to `.claude/skills/`, which the parent harness discovers. When the user wants the skill available in every project rather than this one, write it to the user-level store at `~/.agents/skills/` instead. That path is `<skills-dir>` below.
