...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs the **simplify** skill on the diff before each commit and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. **simplify** is a separate skill, not part of pstack. If you don't have it, ask for the same outcome in plain words. Remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
